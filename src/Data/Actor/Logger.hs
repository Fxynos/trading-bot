...
instance Logger FileLogger where
    trace (FileLogger filePath) tag level message = appendFile filePath $ (++) (formatRecord tag level message) "\n"
...
instance Logger TerminalLogger where
    trace TerminalLogger tag level message = putStrLn $ formatRecord tag level message

-- Utils --

formatRecord :: String -> Level -> String -> String
formatRecord tag level message =
